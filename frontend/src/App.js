...
function AdminProductManager({ products, fetchProducts }) {
  ...
  const [form, setForm] = useState({ name: "", description: "", price: "", discount: "", stock: "" });
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      <form onSubmit={handleSubmit} style={{ display: "flex", gap: 8, marginBottom: 10 }}>
        ...
        {editing && (
          <button type="button" onClick={() => { setEditing(null); setForm({ name: "", description: "", price: "", discount: "", stock: "" }); }}>
            Cancel
          </button>
        )}
      </form>
...
<>
{cart.map((item) => ( <div key={item.id} style={styles.cartItem}> <div> <strong>{item.name}</strong> <p>₹{item.price}</p> </div>
